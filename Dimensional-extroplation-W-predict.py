...
E1 = 410000; aa1 = 4.5e-6; v1 = 0.28  # 目标预测金属为铜
E2 = 70000; aa2 = 6e-7;   v2 = 0.16; h2 = 0.5
E3 = 129000; aa3 = 3e-6;  v3 = 0.28

data = pd.read_excel("WTSV-W-TUOZHAN.xlsx")  
x = data["a"].values / 10
y = data["b"].values / 10 
z = data["c"].values / 100
L1 = data["L"].values / 10
...
class ResidualNet(nn.Module):
    def __init__(self, input_dim=5, hidden_dim=256, output_dim=1): # 🌟 保持 4 维基准波形提取
...
# ==========================================
model = ResidualNet()
model.load_state_dict(torch.load("W-W-GEOM-RESIDUAL2.pth")) 
model.eval()

micro_model = MaterialMicroNet()
micro_model.load_state_dict(torch.load("MICRO-NN-W-WEIGHTS2.pth")) 
micro_model.eval()

scaler_X = joblib.load("scaler_w_W_geom_features2.pkl")
output_csv = "WTSV-W-TUOZHAN-ANALYTICAL-EXTRAPOLATION2.csv"
