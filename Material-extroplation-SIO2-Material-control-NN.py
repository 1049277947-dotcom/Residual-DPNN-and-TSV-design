...
true_fea_stress = torch.tensor([
    108.6865, 123.9258, 146.4241, 162.3892, 216.1638, 
    243.8401, 261.6602, 299.7413, 272.9870, 307.9536
], dtype=torch.float32)
...
for i in range(10):
    ...
    m_theory = calculate_mises_stress(
        current_h1, 2, constants, [E_t, E2, E3], [nu_t, v2, v3], [a_t, aa2, aa3], T
    )
    ...
    K_z, K_theta = get_eshelby_3d_stress(a_val, b_val, nu_t, 1.0, ratio_zr)
...
# =====================================================================
scaler_X = joblib.load("scaler_w_geom_features.pkl")
...
base_nn.load_state_dict(torch.load("W-GEOM-RESIDUAL.pth"))
base_nn.eval()
...
torch.save(micro_nn.state_dict(), "MICRO-NN-WEIGHTS.pth")