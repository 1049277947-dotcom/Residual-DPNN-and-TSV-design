...
for j in [4, 3, 2, 1]:
    ...
    data = pd.read_excel(f"W_Train_Pool_80_10{j}.xlsx",sheet_name="10%") 
    h1 = data["x"] / 1000
    a = data["y"] / 1000
    b = data["z"] / 1000

    # ==========================================
    # 🌟 修改 3：主循环中调用 Eshelby 进行应力合成
    # ==========================================
    ideal_mises_list = []
    Sr_list = [] # 收集径向背景应力
    Sz_list = [] # 收集轴向背景应力
    for i in range(len(data)):
        current_h1 = h1.iloc[i]
        R_current = [current_h1, current_h1 + h2, 5.0]
        
        # 获取宏观的径向应力 A1 和轴向应变 eps_0
        ...
        Sr_list.append(Sr)
        Sz_list.append(Sz)  
        # 🚨 将双轴背景应力喂给 Eshelby 求解器
        Sigma_z, Sigma_theta = get_eshelby_3d_stress(a.iloc[i], b.iloc[i], v1, Sr, Sz)
        
        # 跨尺度缝合：直接使用返回的真实主应力合成 Mises (此时 Sigma_r 为 0)
        ideal_mises = np.sqrt(Sigma_z**2 - Sigma_z * Sigma_theta + Sigma_theta**2)
        ideal_mises_list.append(ideal_mises)

    data["ideal_mises"] = ideal_mises_list
    data["residual"] = data["max"] - data["ideal_mises"]

    # 目标层 y 变为纯净的无量纲残差
    y = (data["residual"] / s).values.reshape(-1, 1)
    ...
    h3 = 5 - h1 - 0.5
    k = (h1 / h3 * E1 / E3 + 1)**0.5
    n1 = ((E2/(2*(1+v2)*E1*h2*h1))**0.5)*L2

    # 🌟 新增：无量纲背景应力特征！除以特征标度 s，强行拉入无量纲空间
    Sr_dimless = np.array(Sr_list) / s
    Sz_dimless = np.array(Sz_list) / s

    X = np.column_stack((a1, b1, k, n1))

    scaler_X = StandardScaler()
    X_scaled = scaler_X.fit_transform(X)
    joblib.dump(scaler_X, f"W_Train_Pool_80_10{j}-mech-residual.pkl") 
    ...
    # 6. 保存模型
    # ==========================================
    torch.save(net.state_dict(), f"w_Train_Pool_80_10{j}-mech-residual.pth") 
    pd.DataFrame(loss_records).to_excel(f"W_Train_Pool_80_10{j}-mech-residual-LOSS.xlsx", index=False)
