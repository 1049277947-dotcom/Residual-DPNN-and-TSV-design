...
data = pd.read_excel("WTSV-SI-TUOZHAN.xlsx")  
x = data["a"].values / 10
y = data["b"].values / 10 
z = data["c"].values / 100
L1 = data["L"].values / 10
...
model = ResidualNet()
model.load_state_dict(torch.load("W-SI-GEOM-RESIDUAL1.pth")) 
model.eval()

# 2. 载入刚训好的 Micro-NN (预测材料本构)
micro_model = MaterialMicroNet()
# 【注意】请确保你在训练代码最后保存了这个权重文件！
micro_model.load_state_dict(torch.load("MICRO-NN-SI-WEIGHTS1.pth")) 
micro_model.eval()

scaler_X = joblib.load("w_SI_geom_features1.pkl")
output_csv = "WTSV-SI-TUOZHAN-ANALYTICAL-EXTRAPOLATION1.csv"
...
with open(output_csv, "w", newline='', encoding='utf-8') as f:
    ...
    for i in range(len(x)):
        ...
        n1_val = ((E2/(2*(1+v2)*E3*h2*h3))**0.5)*L
