...
for j in [4, 3, 2, 1]:
    ...
    data = pd.read_excel(f"SI_Train_Pool_80_10{j}.xlsx",sheet_name="10%") 
    # ==========================================
    # 3. 特征提取与残差目标构建
    # ==========================================
    X_features = []
    ...
    joblib.dump(scaler_X, f"SI_Train_Pool_80_10{j}-mech-residual.pkl") 
    ...
    # 6. 保存模型
    # ==========================================
    torch.save(net.state_dict(), f"SI_Train_Pool_80_10{j}-mech-residual.pth") 
    pd.DataFrame(loss_records).to_excel(f"SI_Train_Pool_80_10{j}-mech-residual-LOSS.xlsx", index=False)
