...
# 读取 20% 标准验证集
val_data = pd.read_excel("SIO2_Validation_Set_20.xlsx")
...
for j in [50, 40, 30, 20, 10]:
    ...
    scaler_X = joblib.load(f"SIO2_Train_Pool_80_{j}-nromal-residual.pkl")
    X_val_scaled = scaler_X.transform(X_val_raw)
    X_val_tensor = torch.tensor(X_val_scaled, dtype=torch.float32)
    
    # 2. 初始化网络并加载对应权重
    net = ResidualNet()
    net.load_state_dict(torch.load(f"SIO2_Train_Pool_80_{j}-nromal-residual.pth"))
    net.eval()
...
origin_df.to_excel("SIO2_Origin_normal-residual2.xlsx", index=False)