...
for index, row in val_data.iterrows():
    ...
    X_val_features.append([current_h1, a_val, b_val])
...
for j in [50, 40, 30, 20, 10]:
    ...
    scaler_X = joblib.load(f"SI_Train_Pool_80_{j}-nromal-residual.pkl")
    X_val_scaled = scaler_X.transform(X_val_raw)
    X_val_tensor = torch.tensor(X_val_scaled, dtype=torch.float32)
    
    # 2. 初始化网络并加载对应权重
    net = ResidualNet()
    net.load_state_dict(torch.load(f"SI_Train_Pool_80_{j}-nromal-residual.pth"))
    net.eval()
...
origin_df.to_excel("SI_Origin_normal-residual2.xlsx", index=False)