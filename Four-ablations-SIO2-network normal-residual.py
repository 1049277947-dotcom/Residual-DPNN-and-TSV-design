...
for j in [50, 40, 30, 20, 10]:
    ...
    data = pd.read_excel("SIO2_Train_Pool_80_sampled2.xlsx",sheet_name=f"{j}%") 
    ...
    for index, row in data.iterrows():
        ...
        X_features.append([current_h1, a_val, b_val])
    ...
    joblib.dump(scaler_X, f"SIO2_Train_Pool_80_{j}-nromal-residual.pkl") 
    ...
    # 6. 保存模型
    # ==========================================
    torch.save(net.state_dict(), f"SIO2_Train_Pool_80_{j}-nromal-residual.pth") 
    pd.DataFrame(loss_records).to_excel(f"SIO2_Train_Pool_80_{j}-nromal-residual-LOSS.xlsx", index=False)
