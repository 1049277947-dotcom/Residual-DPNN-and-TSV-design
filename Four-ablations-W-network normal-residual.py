...
for j in [50, 40, 30, 20, 10]:
    ...
    data = pd.read_excel("W_Train_Pool_80_sampled2.xlsx",sheet_name=f"{j}%")  
    h1 = data["x"] / 1000
    a = data["y"] / 1000
    b = data["z"] / 1000
    ...
    X = np.column_stack((h1,a,b))

    scaler_X = StandardScaler()
    X_scaled = scaler_X.fit_transform(X)
    joblib.dump(scaler_X, f"W_Train_Pool_80_{j}-nromal-residual.pkl") 
    ...
    # 6. 保存模型
    # ==========================================
    torch.save(net.state_dict(), f"W_Train_Pool_80_{j}-nromal-residual.pth") 
    pd.DataFrame(loss_records).to_excel(f"W_Train_Pool_80_{j}-nromal-residual-LOSS.xlsx", index=False)
