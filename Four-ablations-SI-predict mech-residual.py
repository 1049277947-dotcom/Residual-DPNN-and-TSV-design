...
for index, row in val_data.iterrows():
    ...
    mises_theory = calculate_mises_stress(current_h1 + h2, 3, constants, [E1, E2, E3], [v1, v2, v3], [aa1, aa2, aa3], T)
    
    mises_theory_list.append(mises_theory)
    ...
    X_val_features.append([K_theta, K_z, k_val, n1_val])

X_val_raw = np.array(X_val_features)
mises_theory_arr = np.array(mises_theory_list)
...
origin_boxplot_data = {}
for j in [4, 3, 2, 1]:
    # 1. 加载对应的归一化器，对验证集特征进行缩放
    scaler_X = joblib.load(f"SI_Train_Pool_80_10{j}-mech-residual.pkl")
    X_val_scaled = scaler_X.transform(X_val_raw)
    X_val_tensor = torch.tensor(X_val_scaled, dtype=torch.float32)
    
    # 2. 初始化网络并加载对应权重
    net = ResidualNet()
    net.load_state_dict(torch.load(f"SI_Train_Pool_80_10{j}-mech-residual.pth"))
    net.eval()
    ...
    pred_max = mises_theory_arr + pred_residual
    
    # 5. 计算评估指标
    mae = np.mean(np.abs(pred_max - actual_max_arr))                           # 平均绝对误差
    rmse = np.sqrt(np.mean((pred_max - actual_max_arr)**2))                    # 均方根误差
    mape = np.mean(np.abs((pred_max - actual_max_arr) / actual_max_arr)) * 100 # 平均相对误差百分比
...
origin_df.to_excel("SI_Origin_mech-residual4.xlsx", index=False)
