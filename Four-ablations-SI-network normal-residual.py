...
for j in [50, 40, 30, 20, 10]:
    ...
    joblib.dump(scaler_X, f"SI_Train_Pool_80_{j}-nromal-residual.pkl") 
    ...
    # 6. 保存模型
    # ==========================================
    torch.save(net.state_dict(), f"SI_Train_Pool_80_{j}-nromal-residual.pth") 
    pd.DataFrame(loss_records).to_excel(f"SI_Train_Pool_80_{j}-nromal-residual-LOSS.xlsx", index=False)
