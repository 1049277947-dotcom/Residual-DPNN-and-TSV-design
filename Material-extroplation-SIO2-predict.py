...
E1 = 410000; aa1 = 4.58e-6; v1 = 0.28 # 核心已变为铜
E2 = 70000;  aa2 = 6e-7;   v2 = 0.16; h2 = 0.5
E3 = 129000; aa3 = 3e-6;   v3 = 0.28

data = pd.read_excel("W-SIO2.xlsx")  
x = data["x"].values / 10
y = data["y"].values  
z = data["z"].values / 10
...
model = ResidualNet()
model.load_state_dict(torch.load("W-GEOM-RESIDUAL.pth")) 
model.eval()

# 2. 载入刚训好的 Micro-NN (预测材料本构)
micro_model = MaterialMicroNet()
# 【注意】请确保你在训练代码最后保存了这个权重文件！
micro_model.load_state_dict(torch.load("MICRO-NN-WEIGHTS.pth")) 
micro_model.eval()

scaler_X = joblib.load("scaler_w_geom_features.pkl")
output_csv = "W-SIO2-ANALYTICAL-EXTRAPOLATION.csv"
