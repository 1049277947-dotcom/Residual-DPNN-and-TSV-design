...
model = ResidualNet()
model.load_state_dict(torch.load("W-SI-GEOM-RESIDUAL.pth")) 
model.eval()

# 2. 载入刚训好的 Micro-NN (预测材料本构)
micro_model = MaterialMicroNet()
# 【注意】请确保你在训练代码最后保存了这个权重文件！
micro_model.load_state_dict(torch.load("MICRO-NN-SI-WEIGHTS.pth")) 
micro_model.eval()

scaler_X = joblib.load("scaler_w_SI_geom_features.pkl")
output_csv = "W-SI-ANALYTICAL-EXTRAPOLATION.csv"
