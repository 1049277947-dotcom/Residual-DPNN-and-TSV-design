...
for j in [50, 40, 30, 20, 10]:
    ...
    joblib.dump(scaler_X, f"W_Train_Pool_80_{j}-nromal-full.pkl") 
    y = data[["max"]].values
    ...
    loss_df.to_excel(f"W_Train_Pool_80_{j}-LOSS-normal-full.xlsx", index=False)
    print("损失记录已保存到 loss_records.xlsx")

    # ==========================
    # 4. 保存模型（只保存权重）
    # ==========================
    torch.save(net.state_dict(), f"W_Train_Pool_80_{j}_NET-normal-full.pth")
    print("模型训练完成并已保存为 W_Train_Pool_80_60_NET.pth")
