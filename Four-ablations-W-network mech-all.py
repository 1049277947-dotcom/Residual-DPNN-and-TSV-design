...
for j in [50, 40, 30, 20, 10]:
    ...
    joblib.dump(scaler_X, f"W_Train_Pool_80_{j}-mech-all.pkl") 
    ...
    # 6. 保存模型
    # ==========================================
    torch.save(net.state_dict(), f"w_Train_Pool_80_{j}-mech-all.pth") 
    pd.DataFrame(loss_records).to_excel(f"W_Train_Pool_80_{j}-mech-all-LOSS.xlsx", index=False)
