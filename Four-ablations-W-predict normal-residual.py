...
for j in [50, 40, 30, 20, 10]:
    # 1. 加载归一化器，转换验证集
    scaler_X = joblib.load(f"W_Train_Pool_80_{j}-nromal-residual.pkl")
    X_val_scaled = scaler_X.transform(X_val_raw)
    X_val_tensor = torch.tensor(X_val_scaled, dtype=torch.float32)
    
    # 2. 初始化网络并加载对应权重（注意前缀大小写，你的训练代码存的是 w_Train_...）
    net = ResidualNet()
    net.load_state_dict(torch.load(f"w_Train_Pool_80_{j}-nromal-residual.pth"))
    net.eval()
...
origin_df.to_excel("W_Origin_normal-residual2.xlsx", index=False)