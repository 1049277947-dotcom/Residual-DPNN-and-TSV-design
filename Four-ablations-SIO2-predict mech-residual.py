...
for j in [4, 3, 2, 1]:
    ...
    scaler_X = joblib.load(f"SIO2_Train_Pool_80_10{j}-mech-residual.pkl")
    X_val_scaled = scaler_X.transform(X_val_raw)
    X_val_tensor = torch.tensor(X_val_scaled, dtype=torch.float32)
    
    # 2. 初始化网络并加载对应权重
    net = ResidualNet()
    net.load_state_dict(torch.load(f"SIO2_Train_Pool_80_10{j}-mech-residual.pth"))
    net.eval()
...
origin_df.to_excel("SIO2_Origin_mech-residual4.xlsx", index=False)