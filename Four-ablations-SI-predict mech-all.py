...
# ==========================================
pi = 3.1415926536
L1=26; E1=410000; aa1=4.5e-6; v1=0.28
...
for j in [50, 40, 30, 20, 10]:
    ...
    scaler_X = joblib.load(f"SI_Train_Pool_80_{j}-mech-all.pkl")
    X_val_scaled = scaler_X.transform(X_val_raw)
    X_val_tensor = torch.tensor(X_val_scaled, dtype=torch.float32)
    
    # 2. 初始化网络并加载对应权重
    net = ResidualNet()
    net.load_state_dict(torch.load(f"SI_Train_Pool_80_{j}-mech-all.pth"))
    net.eval()
...
origin_df.to_excel("SI_Origin_mech-all2.xlsx", index=False)