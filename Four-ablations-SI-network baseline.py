...
for j in [90, 80, 70, 60]:
    ...
    # 1. 读取数据
    # ==========================
    data = pd.read_excel("SI_Train_Pool_80_sampled.xlsx",sheet_name=f"{j}%")   
    ...
    joblib.dump(scaler_X, f"SI_Train_Pool_80_{j}-nromal-full.pkl") 
    y = data[["max"]].values
    ...
    loss_df.to_excel(f"SI_Train_Pool_80_{j}-LOSS-nromal-full.xlsx", index=False)
    print("损失记录已保存到 loss_records.xlsx")

    # ==========================
    # 4. 保存模型（只保存权重）
    # ==========================
    torch.save(net.state_dict(), f"SI_Train_Pool_80_{j}_NET-nromal-full.pth")
    print("模型训练完成并已保存为 SI_Train_Pool_80_90_NET.pth")
