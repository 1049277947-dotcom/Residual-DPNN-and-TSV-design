...
for j in ratios:
    print(f"================ 开始验证 {j}% 模型 ================")
    # 1. 加载归一化器，转换验证集
    scaler_X = joblib.load(f"SIO2_Train_Pool_80_{j}-nromal-full.pkl")
    X_val_scaled = scaler_X.transform(X_val_raw)
    X_val_tensor = torch.tensor(X_val_scaled, dtype=torch.float32)       
    # 1. 初始化网络并加载对应比例的权重
    net = Net()
    model_path = f"SIO2_Train_Pool_80_{j}_NET-normal-full.pth" 
    ...
    # 2. 进行预测
    with torch.no_grad():
        predictions = net(X_val_tensor).numpy().flatten()
...
origin_df.to_excel("SIO2_Origin-normal-full.xlsx", index=False)
